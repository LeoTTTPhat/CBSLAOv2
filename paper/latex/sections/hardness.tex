...
\begin{tikzpicture}[
  ...
  result/.style={draw=black!60, rounded corners=2pt, thick, align=center,
                 minimum width=34mm, minimum height=12mm, fill=black!3},
  hard/.style={result, fill=red!7, draw=red!60!black},
  poly/.style={result, fill=green!8, draw=green!45!black},
  gap/.style={result, fill=orange!10, draw=orange!65!black},
  arrow/.style={->, thick, draw=black!65}
]
\node[hard] (knapsack) {0/1 Knapsack\\$\Rightarrow$ NP-hard\\Thm.~\ref{thm:hardness}};
\node[hard, right=of knapsack] (coverage) {Max-$k$-Coverage\\$\Rightarrow$ $(1-1/e+\varepsilon)$ hard\\Thm.~\ref{thm:inapprox}};
\node[poly, right=of coverage] (approx) {Offline greedy\\$(1-1/e)$ guarantee\\Thm.~\ref{thm:approx}};
\node[gap, below=of coverage] (gap) {Adaptivity gap\\$\ge 4/3$, $\le$ constant\\Lemma~\ref{lem:adaptivity-gap}};
\draw[arrow] (knapsack) -- node[above]{exact} (coverage);
\draw[arrow] (coverage) -- node[above]{tight up to $\varepsilon$} (approx);
\draw[arrow] (coverage) -- (gap);
\draw[arrow] (gap) -- (approx);
\end{tikzpicture}}
\caption{Theoretical landscape for \CBSLAO. Exact optimisation is
NP-hard, approximation is tight at $1-1/e$ for the offline
monotone-submodular case, and adaptivity improves value by at most a
constant factor in the stochastic-knapsack regime.}
...
\begin{remark}[Strict versus fuzzy schema typing]
\label{rem:fuzzy-schema-lower-bound}
The reduction deliberately uses \emph{strict} schema typing: each
output type $\tau_u$ encodes exact incidence of an element $u$ in a
set $S_j$. This is appropriate for the lower bound but should not be
read as modelling fuzzy MCP/OpenAPI registries directly. If fuzzy
matching is implemented as a deterministic thresholded compatibility
relation, the same construction survives by assigning incidence edges
similarity $1$ and non-incidence edges similarity $0$, so the
$(1-1/e+\varepsilon)$ barrier does not degrade gracefully. If,
instead, fuzzy matching is probabilistic, calibrated, or allowed to
return false positives with nonzero utility, the present reduction no
longer pins down the approximation threshold; a separate lower bound
for probabilistic type compatibility would be needed.
\end{remark}
